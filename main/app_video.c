...
int app_video_open(char *dev, video_fmt_t init_fmt)
{
    ...
    /* Force set format and resolution */
    struct v4l2_format format = {
        .type = type,
        .fmt.pix.width = (init_fmt == APP_VIDEO_FMT_YUV422) ? 320 : default_format.fmt.pix.width,
        .fmt.pix.height = (init_fmt == APP_VIDEO_FMT_YUV422) ? 240 : default_format.fmt.pix.height,
    ...
    ESP_LOGW(TAG, "!!! HARDWARE STATE: %c%c%c%c, %dx%d !!!",
